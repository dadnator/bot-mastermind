...
# Remplacer duels par mastermind_games
mastermind_games = {}
...
async def jouer_mastermind(interaction: discord.Interaction, game_data, original_message):
    joueur = game_data["joueur"]
    montant = game_data["montant"]
    code_secret = game_data["code_secret"]
    tentatives_restantes = game_data["tentatives_restantes"]
    historique_tours = game_data["historique_tours"]
    
    # 1. Créer un embed pour le jeu
    game_embed = discord.Embed(
        title="🧠 Mastermind - En cours",
        description=f"Le code secret a été généré. Tu as **{tentatives_restantes}** tentatives pour le trouver.",
    ...
    if historique_tours:
        for tour in historique_tours:
            historique_str += f"`{tour['proposition']}` : Noirs 🖤: {tour['noirs']} | Blancs 🤍: {tour['blancs']}\n"
        game_embed.add_field(name="Historique", value=historique_str, inline=False)

    await interaction.response.send_message(embed=game_embed)
    game_message = await interaction.original_response()
    mastermind_games[game_message.id] = game_data

    while tentatives_restantes > 0:
        def check(m):
            return m.author.id == joueur.id and m.channel.id == interaction.channel.id and all(c in COULEURS for c in m.content.split()) and len(m.content.split()) == LONGUEUR_CODE
        ...
        try:
            ...
            if noirs == LONGUEUR_CODE:
                ...
                result_embed = discord.Embed(
                    title="🎉 Victoire !",
                    description=f"{joueur.mention} a trouvé le code ! Le code secret était : **{' '.join(code_secret)}**",
                    color=discord.Color.green()
                )
                result_embed.add_field(name="Tentatives", value=f"{len(historique_tours)}/{game_data['tentatives']}", inline=False)
                ...
                # Sauvegarde en BDD
                now = datetime.utcnow()
                c.execute("INSERT INTO mastermind_games (joueur_id, montant, victoire, tentatives, date) VALUES (?, ?, ?, ?, ?)",
                          (joueur.id, montant, 1, len(historique_tours), now))
                conn.commit()
                ...
                game_data["tentatives_restantes"] = tentatives_restantes

                game_embed.description = f"Il te reste **{tentatives_restantes}** tentatives pour le trouver."
                historique_str = ""
                for tour in historique_tours:
            ...
            result_embed = discord.Embed(
                title="❌ Échec",
                description=f"{joueur.mention} n'a pas répondu à temps.",
    ...
    if tentatives_restantes == 0:
        ...
        result_embed = discord.Embed(
            title="❌ Défaite !",
            description=f"{joueur.mention} a échoué. Le code secret était : **{' '.join(code_secret)}**",
            color=discord.Color.red()
        )
        result_embed.add_field(name="Tentatives", value=f"0/{game_data['tentatives']}", inline=False)
        ...
        now = datetime.utcnow()
        c.execute("INSERT INTO mastermind_games (joueur_id, montant, victoire, tentatives, date) VALUES (?, ?, ?, ?, ?)",
                  (joueur.id, montant, 0, len(historique_tours), now))
        conn.commit()
    
    mastermind_games.pop(game_message.id, None)

# --- CLASSES ET COMMANDES ---
class MastermindView(discord.ui.View):
    def __init__(self, message_id, joueur, montant):
    ...
    async def rejoindre_croupier(self, interaction: discord.Interaction):
        ...
        self.clear_items()
        self.lancer_game_button = discord.ui.Button(label="🎰 Lancer le Mastermind !", style=discord.ButtonStyle.success, custom_id="lancer_game")
        ...
        embed.description = f"Le joueur {self.joueur.mention} est prêt pour une partie à **{format(self.montant, ',').replace(',', ' ')}** kamas."
    ...
    async def lancer_game(self, interaction: discord.Interaction):
        if interaction.user.id != self.croupier.id:
            await interaction.response.send_message("❌ Seul le croupier peut lancer le jeu.", ephemeral=True)
            return
        ...
        await jouer_mastermind(interaction, game_data, original_message)


# --- COMMANDES ---

@bot.tree.command(name="mastermind", description="Lancer un duel de Mastermind.")
...
async def mastermind_game(interaction: discord.Interaction, montant: int):
    ...
    for game_data in mastermind_games.values():
        if game_data["joueur"].id == interaction.user.id:
            await interaction.response.send_message(
                "❌ Tu participes déjà à une partie de Mastermind. Termine-la ou utilise `/quit` pour l'annuler.",
                ephemeral=True)
            return
    
    code_secret = random.choices(COULEURS, k=LONGUEUR_CODE)
    
    embed = discord.Embed(
        title="🧠 Nouvelle Partie de Mastermind",
        description=f"**{interaction.user.mention}** lance un défi pour **{montant:,.0f}".replace(",", " ") + " kamas** 💰\n"
                      "Un croupier est attendu pour lancer la partie.",
    ...
    ping_content = ""
    if role_croupier:
        ping_content = f"{role_croupier.mention} — Un nouveau défi Mastermind est prêt !"
    ...
    mastermind_games[sent_message.id] = {
...
@bot.tree.command(name="quit", description="Annule ta partie de Mastermind en cours.")
async def quit_game(interaction: discord.Interaction):
    ...
    if game_a_annuler_id is None:
        await interaction.response.send_message(
            "❌ Tu n'as aucune partie de Mastermind en attente à annuler.", ephemeral=True)
        return
    ...
    try:
        ...
        embed_initial.title = "❌ Partie annulée"
        embed_initial.description = f"La partie de **{game_data['joueur'].display_name}** a été annulée."
        embed_initial.color = discord.Color.red()
        await message_initial.edit(embed=embed_initial, view=None, content="")
    except Exception:
        pass
    await interaction.response.send_message("✅ Ta partie a bien été annulée.", ephemeral=True)
...
async def stats_mastermind(interaction: discord.Interaction):
    ...
    SELECT joueur_id,
           ...
           SUM(CASE WHEN victoire = 1 THEN montant * 2 * 0.95 ELSE 0 END) as kamas_gagnes
    FROM mastermind_games
    GROUP BY joueur_id
    """)
    data = c.fetchall()
