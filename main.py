...
class MastermindGameView(discord.ui.View):
    ...
    def add_buttons(self):
        ...
        validate_button = discord.ui.Button(label="✅ Valider", style=discord.ButtonStyle.green, custom_id="validate_guess", row=1)
        validate_button.callback = self.validate_guess
        self.add_item(validate_button)
        
    def update_embed(self):
        joueur1 = self.game_data['joueur1']
        joueur2 = self.game_data['joueur2']
        
        embed = discord.Embed(
            title="🧠 Mastermind - En cours",
            description=f"**{joueur2.mention}**, il te reste **{self.tentatives_restantes}** tentatives pour trouver le code de **{joueur1.mention}**.",
        ...
        if not self.historique_tours:
            ...
            for tour in self.historique_tours:
                historique_str += f"`{' '.join(tour['proposition'])}` : Noirs 🖤: {tour['noirs']} | Blancs 🤍: {tour['blancs']}\n"
        embed.add_field(name="Historique", value=historique_str, inline=False)

        proposition_str = " ".join(self.proposition_actuelle)
        embed.add_field(name="Ta proposition actuelle", value=f"`{proposition_str}`", inline=False)
    ...
    async def add_color_guess(self, interaction: discord.Interaction):
        if interaction.user.id != self.game_data['joueur2'].id:
        ...
        if len(self.proposition_actuelle) < LONGUEUR_CODE:
            try:
                ...
                if button_clicked:
                    self.proposition_actuelle.append(button_clicked.emoji.name)
                    await interaction.response.edit_message(embed=self.update_embed(), view=self)
                else:
                    await interaction.response.send_message("❌ Ce bouton n'est pas actif.", ephemeral=True)
            except Exception:
                await interaction.response.send_message("❌ Ce bouton n'est plus actif.", ephemeral=True)
        else:
            await interaction.response.send_message(f"❌ La proposition ne peut pas avoir plus de {LONGUEUR_CODE} couleurs.", ephemeral=True)

    async def clear_guess(self, interaction: discord.Interaction):
        if interaction.user.id != self.game_data['joueur2'].id:
            await interaction.response.send_message("❌ Ce n'est pas ton tour de jouer.", ephemeral=True)
            return
    ...
    async def validate_guess(self, interaction: discord.Interaction):
        if interaction.user.id != self.game_data['joueur2'].id:
            await interaction.response.send_message("❌ Ce n'est pas ton tour de jouer.", ephemeral=True)
            return
            
        if len(self.proposition_actuelle) != LONGUEUR_CODE:
            await interaction.response.send_message(f"❌ Veuillez choisir {LONGUEUR_CODE} couleurs pour valider.", ephemeral=True)
            return

        code_secret = self.game_data['code_secret']
        noirs, blancs = calculer_indices(code_secret, self.proposition_actuelle)
        
        self.historique_tours.append({
        ...
        if noirs == LONGUEUR_CODE:
            gagnant = self.game_data['joueur2']
            await self.end_game(gagnant)
    ...
    async def end_game(self, gagnant):
        joueur1 = self.game_data['joueur1']
        joueur2 = self.game_data['joueur2']
        montant = self.game_data['montant']
        code_secret = self.game_data['code_secret']
        ...
        c.execute("INSERT INTO mastermind_games (joueur1_id, joueur2_id, montant, gagnant_id, tentatives, date) VALUES (?, ?, ?, ?, ?, ?)",
                  (joueur1.id, joueur2.id, montant, gagnant_id, len(self.historique_tours), now))
        conn.commit()
        
        mastermind_games.pop(self.game_data["original_message_id"], None)
        self.stop()
...
async def jouer_mastermind(interaction: discord.Interaction, game_data):
    ...
    initial_embed.add_field(name="Couleurs disponibles", value=" | ".join(COULEURS), inline=False)
    initial_embed.add_field(name="Historique", value="`Aucune proposition pour le moment.`", inline=False)
    initial_embed.add_field(name="Ta proposition actuelle", value="` `", inline=False)
...
# --- VUE POUR LE CHOIX DU CODE SECRET (JOUEUR 1) ---
class SecretCodeView(discord.ui.View):
    ...
    async def add_color(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message("❌ Ce bouton n'est plus actif ou il y a une erreur dans le code. Contacte l'administrateur.", ephemeral=True)
            try:
                await interaction.message.delete()
            except discord.errors.NotFound:
                pass
...
# --- VUES ET COMMANDES (sans changement majeur) ---
class MastermindView(discord.ui.View):
    ...
    async def start_game_callback(btn_interaction: discord.Interaction):
        ...
        # We update the message to reflect the disabled button.
        await btn_interaction.response.edit_message(view=start_game_view)

        # We send the ephemeral message using followup.
        await btn_interaction.followup.send_message(
...
await interaction.response.defer()
    ...
    # We update the game data with the new message ID.
    mastermind_games.pop(self.message_id, None)
    game_data["original_message_id"] = message_to_send_joueur1.id
    mastermind_games[message_to_send_joueur1.id] = game_data
